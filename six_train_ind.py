...
from six_ttlora_wrapper import TTLoRALinearWrapper_withcores
    ...
    if model_name == "roberta-base":
        for layer in model.roberta.encoder.layer:
            if ttlora_adapter_at_query:
                ...
                layer.attention.self.query = assign_ttlora(layer.attention.self.query, layer_weight= layer.attention.self.query.weight) #layer is sent as module
            if ttlora_adapter_at_value:
                # value_weight_copy = layer.attention.self.value.weight.clone()
                layer.attention.self.value = assign_ttlora(layer.attention.self.value, layer_weight=layer.attention.self.value.weight) #layer is sent as module
...
def main():
    ...
    print(df)
    filename = f"AdaptWithCores_{dataset_name}_{model_name}.csv"
