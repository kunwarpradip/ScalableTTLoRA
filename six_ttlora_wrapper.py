...
def tensorized_multiplication(X, tt_cores, m_factors, n_factors, layer_weight_copy, alpha, device):
    """
    ...
    tt_shape = m_factors + n_factors
    r= 4
    tt_rank = get_ttlora_rank(r, tt_shape)
    layer_weight = tl.reshape(layer_weight_copy, tt_shape).to(device)
    layer_weight_cores = tensor_train(layer_weight, tt_rank)
    ...
    for i in range(num_m):
        ...
        initial_core = tt_cores[i]  # shape [r_in, m_i, r_out]
    ...
    for i in range(num_n):
        initial_core = tt_cores[start_n + i]  # shape [r_in, n_i, r_out]
...
class TTLoRALinearWrapper_withcores(nn.Module): #Inherits from nn.Module
        def __init__(self, module: nn.Module, tt_shape, tt_rank, alpha:int, layer_weight:torch.Tensor):
            super().__init__()
            self.base_module = module
            self.tt_shape = tt_shape
            self.tt_rank = tt_rank
            self.alpha=alpha
            self.layer_weight_copy = layer_weight
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor: # x is input used in forward pass at every call of model
            if self.alpha > 0:
                ...
                out = tensorized_multiplication(x.to(device), 
                                                ...
                                                m_factors=m_factors, 
                                                n_factors=n_factors, 
                                                layer_weight_copy=self.layer_weight_copy.to(device), 
                                                alpha=self.alpha, 
                                                device=device) 
